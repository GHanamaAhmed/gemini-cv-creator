...
import {
  generateCVContentWithTemplate,
  generateCVContentWithTemplate2,
} from "./googleClient";
import fs from "fs";
import { takeScreenshot } from "./puppeteer";
const handlebars = require("handlebars");
const signature = fs.readFileSync(path.join(process.cwd(), "cv.html"), "utf-8");
const template = handlebars.compile(signature);
const htmlToSend = (content: string) => {
  let replacements = {
    content: content,
  };
...
<div class="container">
  ...
  const res = await generateCVContentWithTemplate2(
    ...
    "3 years at Company X working on web applications",
    "Bachelor's in Computer Science from University Y",
